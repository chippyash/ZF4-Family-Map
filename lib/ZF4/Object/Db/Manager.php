...
class ZF4_Object_Db_Manager {
    ...
    protected static function _loadCache() {
        if (!self::$_isCacheLoaded) {
            ...
            $cacheObj = self::_fileGetContents($cacheFile);
            if ($cacheObj === false) {
                return;
            } else {
                $cacheObj = unserialize($cacheObj);
            }
    ...
    public static function _saveCache() {
        if (self::$_isCacheLoaded) {
            ...
            self::_filePutContents($cacheFile, serialize($cacheObj));
            self::$_isCacheLoaded = false;
        }
    }

    /**
     * Clear the db object cache
     * 
     */
    ...
    /**
     * Set shutdown function to save the cache
     */
    protected static function _setSaveCache() {
        if (!self::$_doSaveCache) {
            //register the saveCache function for shutdown processing
            register_shutdown_function(array('ZF4_Object_Db_Manager', '_saveCache'));
            self::$_doSaveCache = true;
        }
    }
     ...
     */
    protected static function _fileGetContents($file)
    {
        $result = false;
        if (!is_file($file)) {
            return false;
     ...
     */
    protected static function _filePutContents($file, $string)
    {
        $result = false;
        $f = @fopen($file, 'ab+');
        if ($f) {
