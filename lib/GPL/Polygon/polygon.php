...
** Modifications: Ashley Kitson, ZWare Limited UK, to bring in line with PHP 5
**------------------------------------------------------------------------------
...
*/
	protected $_first;		// Reference to first vertex in the linked list
	...
	protected $_cnt = 0;		// Tracks number of vertices in the polygon
	/*
	...
	*/
	function print_poly()
	{
		...
		while ($c->id() != $this->_first->id());
		if ($this->_first->nextPoly)
		{
			print("Next Polygon:<br>");
			$this->_first->nextPoly->print_poly();
		}
	}
	/*
	...
	*/
	function add (&$nv)
	{
		if ($this->_cnt == 0)				// If this is the first vertex in the polygon
		{
			$this->_first =& $nv;			// Save a reference to it in the polygon
			$this->_first->setNext($nv);		// Set its pointer to point to itself
			$this->_first->setPrev($nv);		// because it is the only vertex in the list
			$ps =& $this->_first->Nseg();	// Get ref to the Next segment object
			$this->_first->setPseg($ps);		// and save it as Prev segment as well
		}
		else								// At least one other vertex already exists
		{
			// $p <-> $nv <-> $n
			//    $ps     $ns
			$n =& $this->_first;				// Get a ref to the first vertex in the list
			...
			$n->setPseg($ns);				// Set new previous seg for $this->_first
			$nv->setPseg($ps);				// Set previous seg of the new vertex
		}
		$this->_cnt++;						// Increment the count of vertices
	}
	/*
	...
	*/
	function insertSort (&$nv, &$s, &$e)
	{
		...
		$ns =& $nv->Nseg();
		$c->setPseg($ns);
		$this->_cnt++;					// Just added a new vertex
	}
	/*
	...
	*/
	function &first_unckd_intersect ()
	{
		...
		while($v->id() != $this->_first->id() && ( !$v->isIntersect() || ( $v->isIntersect() && $v->isChecked() ) ) );
		return $v;
	}
	/*
...
//					print("Perturb: P(".$p1->X().",".$p1->Y().")(".$p2->X().",".$p2->Y().") Q(".$q1->X().",".$q1->Y().")(".$q2->X().",".$q2->Y().") UA(".$ua.") UB(".$ub.")<br>");
	...
	*/
	function isInside (&$v)
	{
		...
		$q =& $this->_first;		// End vertex of a line segment in polygon
		do
		{
			if (!$q->isIntersect())
			{
	...
	*/
	...
	{
		$last = NULL;
		$s =& $this->_first;			// First vertex of the subject polygon
		...
		*/
		...
		{
			$v =& $this->_first_unckd_intersect();	// Get the first unchecked intersect point
			$this_class = get_class($this);			// Findout the class I'm in
	...
	**
	** A|B : Return $this with $polyB in $this->_first->nextPoly
	** A&B : Return $this
	** A\B : Return $this
	** B\A : return $polyB
	*/
		...
		elseif ($this->_first->nextPoly)
		{
			$last->first->nextPoly =& $this->_first->NextPoly();
		}
		return $last;
	...
	*/
	function &bRect ()
	{
        ...
        do
		{
		    $v = $p->getFirst();	// Get the first vertex
		    do
		    {
