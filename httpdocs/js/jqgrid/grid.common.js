...
var viewModal = function (selector,o){
	...
	if(jQuery.fancybox){
		...
		var content = jQuery('<div>').append(jQuery(selector).attr("aria-hidden","false").clone());
		//alert(content.find('#cData').first().html());
		
		// make into buttons
		content.find('#cData, #sData').removeClass().addClass('button ok');
		// remove icon tags
		content.find('#cData span, #sData span').remove();
		addButtonIcons();
		
		jQuery.fancybox(content.remove().html(),{
			'width'				: 400
			,'height'			: 450
			,'autoDimensions'	: false
			,onComplete			: function(e){
				//jQuery('#fancybox-inner').find('select').selectmenu();
				jQuery('#fancybox-inner').find('#cData').click(function(){jQuery.fancybox.close();});
			}
		});
